...
inline int calculate_delta_time_ms(const timespec& abs_timeout, const timespec& time_current) {
...
inline ssize_t mq_timedreceive_monotonic(mqd_t mqdes, char* msg_ptr, size_t msg_len, unsigned int* msg_prio, const struct timespec* abs_timeout) {
...
inline int mq_timedsend_monotonic(mqd_t mqdes, const char* msg_ptr, size_t msg_len, unsigned msg_prio, const struct timespec* abs_timeout) {
    if (!is_timetout_valid(abs_timeout)) {
        errno = EINVAL;
        return -1;
    }
